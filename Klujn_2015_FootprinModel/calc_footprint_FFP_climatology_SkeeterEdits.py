def FFP_climatology(zm=None, z0=None, umean=None, h=None, ol=None, sigmav=None, ustar=None,
    ...
    FFP output
        ...
        rr       = Number of parts for countor line of r (includes interior rings!)
    ...
    if not isinstance(zm, list): zm = [zm][0]
    if not isinstance(h, list): h = [h][0]
    if not isinstance(ol, list): ol = [ol][0]
    if not isinstance(sigmav, list): sigmav = [sigmav][0]
    if not isinstance(ustar, list): ustar = [ustar][0]
    if not isinstance(wind_dir, list): wind_dir = [wind_dir][0]
    if not isinstance(z0, list): z0 = [z0][0]
    if not isinstance(umean, list): umean = [umean][0]
    ...
    if n==0:
        ...
        if rs is not None:
            ...
            for ix, fr in enumerate(frs):
                xr,yr,rr = get_contour_vertices(x_2d, y_2d, fclim_2d, fr,rs[ix])
                if xr is None:
                    frs[ix]  = None
                    flag_err = 2
                xrs.append(xr)
            ...
            if crop:
                ...
                yrs = []
                rrs = []
                xrs,yrs,rrs = get_contour_vertices(x_2d, y_2d, fclim_2d, clevs[0][2],rs_dummy)
    ...
    if rs is not None:
        return {'x_2d': x_2d, 'y_2d': y_2d, 'fclim_2d': fclim_2d,
                'rs': rs, 'fr': frs, 'xr': xrs, 'yr': yrs, 'rr': rrs, 'n':n, 'flag_err':flag_err}
    else:
        return {'x_2d': x_2d, 'y_2d': y_2d, 'fclim_2d': fclim_2d,
...
def get_contour_vertices(x, y, f, lev,r):
    import numpy as np
    # import matplotlib._cntr as cntr ## This  module is depreciated, replaced with contours package
    ...
    y=y[:,0]
    # print(x,y)
    c = QuadContourGenerator.from_rectilinear(x, y, f)

    contour = c.filled_contour(min=lev)# for r in thresh]#max=1.0)]
    # plt.figure()
    # for i,A in enumerate(contour):
    #     print(A.shape)
        # plt.plot(A[:,0],A[:,1],label=str(rs[j]))
    ...
    for seg in contour:
        # N = len(seg[:, 0])
        # X = [seg[ix, 0] for ix in range(N)] 
        # Y = [seg[ix, 1] for ix in range(N)] 
        xr.append(seg[:,0])
        yr.append(seg[:,1])
...
#            print (min(xR), max(xR),min(yR),max(yR))
            return [None, None,None]
#    print (rr,'dkadf')
#    print (xr,'2')
    return [xr, yr, rr]   # x,y coords of contour points & corresponding r level.	
